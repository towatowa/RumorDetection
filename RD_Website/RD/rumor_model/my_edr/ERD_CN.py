...
class pooling_layer2(nn.Module):
    ...
    def forward(self, inputs, cuda=False):
        inputs_sent = [torch.cat(
            [self.linear(sent_tensor if cuda else sent_tensor).max(axis=0)[0].unsqueeze(0) for sent_tensor in
             seq]) for seq in inputs]
...
class RDM_Model_V1(nn.Module):
    ...
    def forward(self, input_x, seq_lens):
        ...
        """
        batchsize, max_seq_len, emb_dim = input_x.shape
        h0 = torch.zeros([1, batchsize, self.hidden_dim], dtype=torch.float32)
        c0 = torch.zeros([1, batchsize, self.hidden_dim], dtype=torch.float32)
