...
def get_stopword():
    """获取停用词"""
    stopwords = pd.read_csv("./data/stop_words-master/english.txt", index_col=False, sep="\t", quoting=3,
...
def drop_stopwords(srctweet, stopwords):
    ...
    """
    punc = '\]~`!#$%^&*()_+-=|\';"":/.,?><~·！@#￥%……&*（）——+-=“：’；、。，？》《{}1234567890→✔️\[✏'
    data_dict = json.loads(srctweet)
    srctweet = data_dict['text']
...
def get_net():
    ...
    net = torch.load(r"D:\Pycharm\RD_Website\RumorDetectionApp1\data\LSTM_net_loss=0.0498_acc=0.826.pt")
    net = net.to(device)
